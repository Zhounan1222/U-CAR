...
class SwinCLIP(pl.LightningModule):
    """
    ...
    def __init__(self, args):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("/workspace/ckpt/Bio_ClinicalBERT")
        if self.tokenizer.bos_token_id is None:
            self.tokenizer.bos_token_id = self.tokenizer.cls_token_id
        self.text_encoder = AutoModel.from_pretrained("/workspace/ckpt/Bio_ClinicalBERT")    
    ...
    def training_step(self, batch, batch_idx):
        result = self(batch)
        # # 检查未使用的参数
        # unused_params = [name for name, param in self.named_parameters() if param.grad is None and param.requires_grad]
        # if unused_params:
        #     print("WARNING: Unused parameters:", unused_params)
